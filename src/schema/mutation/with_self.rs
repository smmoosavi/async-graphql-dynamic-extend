...
impl<'a> From<&'a Query> for CountQuery<'a> {
...
impl CountQuery<'_> {
    async fn resolve_count(self, _ctx: &Context<'_>) -> i32 {
        self.parent().db.get_count()
    }
}

impl ExpandObject for CountQuery<'_> {
    type Target = Query;
}

impl Register for CountQuery<'_> {
    fn register(registry: Registry) -> Registry {
        // define count field
        let count_field =
            dynamic::Field::new("count", dynamic::TypeRef::named_nn(TypeRef::INT), |ctx| {
                dynamic::FieldFuture::new(async move {
                    ...
                    let value = CountQuery::resolve_count(parent.into(), &ctx).await;
...
impl Object for MutationRoot<'_> {
    const NAME: &'static str = "Mutation";
}

impl Mutation for MutationRoot<'_> {
    type Root = Root;
}

impl Register for MutationRoot<'_> {
    fn register(registry: Registry) -> Registry {
...
struct CountMutations<'a>(&'a MutationRoot<'a>);

impl<'a> ExpandObject for CountMutations<'a> {
    type Target = MutationRoot<'a>;
}

impl<'a> CountMutations<'a> {
    fn parent(&self) -> &'a MutationRoot<'a> {
        self.0
    }
    fn root(&self) -> &'a Root {
        self.parent().root()
...
impl<'a> CountMutations<'a> {
    async fn resolve_increment(self, _ctx: &Context<'_>) -> i32 {
        self.root().db.increment();
        self.root().db.get_count()
    }
    async fn resolve_decrement(self, _ctx: &Context<'_>) -> i32 {
        self.root().db.decrement();
        self.root().db.get_count()
    }
}

impl Register for CountMutations<'_> {
    fn register(registry: Registry) -> Registry {
        ...
        let increment_field = dynamic::Field::new(
            ...
            |ctx| {
                dynamic::FieldFuture::new(async move {
                    ...
                    let value = CountMutations::resolve_increment(
                        CountMutations(&MutationRoot(parent)),
                        &ctx,
                    )
                    .await;
                    ResolveOwned::resolve_owned(value, &ctx)
        ...
        let decrement_field = dynamic::Field::new(
            ...
            |ctx| {
                dynamic::FieldFuture::new(async move {
                    ...
                    let value = CountMutations::resolve_decrement(
                        CountMutations(&MutationRoot(parent)),
                        &ctx,
                    )
                    .await;
