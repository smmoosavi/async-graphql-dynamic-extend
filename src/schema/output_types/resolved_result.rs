...
impl Foo {
    async fn resolve_bar(&self) -> Result<Bar, MyError> {
        if self.error {
            Err(MyError::Foo)
        } else {
            Ok(Bar {
    ...
    async fn resolve_the_string(&self) -> Result<String, MyError> {
        if self.error {
            Err(MyError::Foo)
        } else {
            Ok("the string".to_string())
        }
    }

    async fn resolve_the_i32(&self) -> Result<i32, MyError> {
        if self.error {
            Err(MyError::Foo)
        } else {
            Ok(42)
    ...
    async fn resolve_the_f32(&self) -> Result<f32, MyError> {
        if self.error {
            Err(MyError::Foo)
        } else {
            Ok(42.0)
    ...
    async fn resolve_the_bool(&self) -> Result<bool, MyError> {
        if self.error {
            Err(MyError::Foo)
        } else {
            Ok(true)
    ...
    async fn resolve_the_id(&self) -> Result<ID, MyError> {
        if self.error {
            Err(MyError::Foo)
        } else {
            Ok(ID("the id".to_string()))
        }
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_query_error_strings() {
        let schema = create_schema();
        let query = r#"
            query {
              foo { the_string the_id }
    ...
    #[tokio::test]
    async fn test_query_error_scalars() {
        let schema = create_schema();
        let query = r#"
            query {
              foo { the_i32 the_f32 the_bool }
